...
// New schema for Better Status

const user = pgTable("user", {
  id: uuid("id").primaryKey().defaultRandom(), // uuid eg: 123e4567-e89b-12d3-a456-426614174000
  name: text("name").notNull(), // name eg: John Doe
  email: text("email").notNull().unique(), // email eg: john.doe@example.com
  password: text("password"), // optional eg: password123
  role: text("role").notNull().default("user"), // role eg: user, admin
  provider: text("provider").notNull().default("email"), // provider eg: email, credentials and google
  is_active: boolean("is_active").default(true).notNull(), // is_active eg: true
  is_verified: boolean("is_verified").default(false).notNull(), // is_verified eg: false
  avatar_url: text("avatar_url"), // optional eg: https://example.com/avatar.png
});

const workspace = pgTable("workspace", {
  id: uuid("id").primaryKey().defaultRandom(), // uuid eg: 123e4567-e89b-12d3-a456-426614174000
  name: text("name").notNull(), // name eg: Workspace 1
  user_id: uuid("user_id")
    .notNull()
    .references(() => user.id, { onDelete: "cascade" }), // user_id eg: 123e4567-e89b-12d3-a456-426614174000
  status: text("status").notNull().default("active"), // status eg: active, paused, failed, completed
  is_active: boolean("is_active").default(true).notNull(), // is_active eg: true
  description: text("description"), // optional eg: "This is a workspace"
  logo_url: text("logo_url"), // optional eg: https://example.com/logo.png
  website_url: text("website_url"), // optional eg: https://example.com
  created_at: timestamp("created_at").defaultNow().notNull(), // created_at eg: 2021-01-01 00:00:00
  updated_at: timestamp("updated_at").defaultNow().notNull(), // updated_at eg: 2021-01-01 00:00:00
});

const job = pgTable("job", {
  id: uuid("id").primaryKey().defaultRandom(), // uuid eg: 123e4567-e89b-12d3-a456-426614174000
  user_id: uuid("user_id")
    .notNull()
    .references(() => user.id, { onDelete: "cascade" }), // user_id eg: 123e4567-e89b-12d3-a456-426614174000
...
const job_log = pgTable("job_log", {
  id: uuid("id").primaryKey().defaultRandom(), // uuid eg: 123e4567-e89b-12d3-a456-426614174000
  job_id: uuid("job_id")
    .notNull()
    .references(() => job.id, { onDelete: "cascade" }), // job_id eg: 123e4567-e89b-12d3-a456-426614174000
  status_code: integer("status_code"), // status_code eg: 200
  response_body: text("response_body"), // response_body eg: { "name": "John Doe" }
  error_message: text("error_message"), // error_message eg: "Error message"
  duration_ms: integer("duration_ms"), // duration_ms eg: 1000
  ...
  succeed_at: timestamp("succeed_at"), // succeed_at eg: 2021-01-01 00:00:00
  failed_at: timestamp("failed_at"), // failed_at eg: 2021-01-01 00:00:00
  run_at: timestamp("run_at").defaultNow().notNull(), // run_at eg: 2021-01-01 00:00:00
});
...
const monitoring = pgTable("monitoring", {
  ...
  workspace_id: uuid("workspace_id")
    .notNull()
    .references(() => workspace.id, { onDelete: "cascade" }), // workspace_id eg: 123e4567-e89b-12d3-a456-426614174000
